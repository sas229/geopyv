...
for i in np.arange(10, 110, 10):
    ...
    subset = geopyv.subset.Subset(
        f_img=ref, g_img=tar, f_coord=f_coord, template=template
    )
    name = "Square_{}_px".format(i)
    save = "./docs/scripts/" + name + ".png"
    subset.inspect(save=save, show=False, block=False)
    img_list.append(save)
    square_metrics.append(
        [
            subset.data["template"]["size"],
            subset.data["quality"]["sigma_intensity"],
            subset.data["quality"]["SSSIG"],
        ]
    )

# Make gif of subset.
os.system(
    "convert -loop 0 -delay 100 %s ./docs/source/images/square_subset_size.gif"
    % " ".join(img_list)
)
...
# Make gif.
os.system(
    "convert -loop 0 -delay 100 %s ./docs/source/images/circle_subset_size.gif"
    % " ".join(img_list)
)
